...
import { agentManager, conversationManager, deliverableManager, knowledgeBaseManager } from '@/lib/agent-system';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (taskId) {
      try {
        const { taskManager } = await import('@/lib/agent-system');
    ...
    if (agent.knowledgeBaseId) {
      try {
        const knowledgeBase = await knowledgeBaseManager.getKnowledgeBaseById(agent.knowledgeBaseId);
        ...
        try {
          ...
          try {
            await conversationManager.createConversation({
              ...
              modelUsed: 'doubao-seed',
            });

            // 如果有任务ID，将对话保存为任务成果
            if (taskId && agent) {
              await deliverableManager.createDeliverable({
                taskId,
                agentId,
          ...
          controller.enqueue(`data: ${JSON.stringify({ type: 'error', content: `\n\n❌ 发生错误: ${error instanceof Error ? error.message : '未知错误'}\n` })}\n\n`);
          controller.enqueue('data: [DONE]\n\n');
        }
      },
    });
    ...
    return new Response(
      JSON.stringify({ error: '对话失败', details: error instanceof Error ? error.message : String(error) }),
